function sendToSalesforce(event){
  Office.context.mailbox.item.from.getAsync((res)=>{
    ...
    const email = from.emailAddress || from.address || '';
    const name = from.displayName || '';
    Office.context.ui.displayDialogAsync(
      window.location.origin + '/addin/src/taskpane.html?email=' + encodeURIComponent(email) + '&name=' + encodeURIComponent(name),
      ...
      try{
        ...
        if(data.type==='submitCharter'){
          ...
          if (toRecipients.length && typeof toRecipients[0] === 'object') {
            to = toRecipients[0].emailAddress || toRecipients[0].address || '';
          }
          const subject = item.subject || '';
          await fetch('/api/sf/log-activity', {
            method:'POST', headers:{'Content-Type':'application/json'}, credentials:'include',
            body: JSON.stringify({ email: to, subject, charter: data.charter })
          });
          dlg.close();
